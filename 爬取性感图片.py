...
class MZiTu:
    ...
    def __init__(self):
        ...
        if a == 1:
            ...
            search = input("2：")
            self.index_url = "https://www.mzitu.com/search/" + \
    ...
    def mkdir(self, dir_name, img_url_list):
        ...
        finall_dir = meizi_dir + '/' + \
            '[{}P]'.format(str(total_image)) + dir_name
        if os.path.isdir("D:\BaiduYunDownload\图片/" + finall_dir) != True:  # 如果保存的路径不存在
            os.makedirs(r"D:\BaiduYunDownload\图片/" +
                        finall_dir)  # 如果不存在。我们将创立这个路径
        path = "D:\BaiduYunDownload\图片/" + finall_dir + "/"
        return path
